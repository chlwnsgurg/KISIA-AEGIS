...
def main(ckpt_dir, instance_image_file, wm_data_dir, device="cuda:0",):
    ...
    bitstream = ''.join(str(int(bit.item())) for bit in message[0])
    integer = int(bitstream, 2)
    identifier = str(integer)
    image_path = os.path.join(instance_image_file)
    ...
    save_image_for_tensor(image[0], f"{wm_data_dir}/{img_name}_{identifier}_orig.png")
    save_image_for_tensor(wm_image[0], f"{wm_data_dir}/{img_name}_{identifier}_wm.png")
    # save_image_for_tensor(residual_image[0], f"{wm_data_dir}/{img_name}_{identifier}_res.png")
